        ...
        ILogger<ProductService> _logger)
{
    ...
    public async Task<IEnumerable<ProductInfo>> GetProductsAsync(string? categoryFilter = null)
    {
        ...
        try
        {
            ...
            var allProducts = await _productCollection.GetAsync(product => product.Name != "", top: 1000).ToListAsync();
    ...
    private async Task EnsureProductsExistAsync()
    {
        try
        {
            ...
            var existingProducts = await _productCollection.GetAsync(product => product.Name != "", top: 1).ToListAsync();
    ...
    private async Task<List<string>> GetUniqueFileNamesAsync()
    {
        try
        {
            ...
            var allChunks = await _chunkCollection.GetAsync(chunk => chunk.DocumentId != "", top: 1000).ToListAsync();
